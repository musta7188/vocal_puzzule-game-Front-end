...
function fetchCards() {
  return fetch(CARDS_URL).then((resp) => resp.json());
}
...
function createPlayer(player_name) {
  ...
  return fetch(PLAYER_URL, data).then((resp) => resp.json());
}
...
function getAllGames() {
  return fetch(GAME_URL).then((resp) => resp.json());
}
...
function showResults(gameObj) {
  ...
  gameContainer.append(br, boardBtn);
...
function appendGame(game, index) {
  ...
  divPlayer.innerText = `${index + 1}. ${game.player.name},  score: ${
    game.score
  } `;
